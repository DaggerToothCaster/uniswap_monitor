...
mod tests {
    ...
    #[test]
    fn test_is_quote_token() {
        let tool = TokenReorderingTool::with_default_config();
        
        assert!(tool.is_quote_token(&Some("KTO".to_string())));
        assert!(tool.is_quote_token(&Some("kto".to_string())));
        assert!(tool.is_quote_token(&Some("USDT".to_string())));
        assert!(tool.is_quote_token(&Some("NOS".to_string())));
        assert!(!tool.is_quote_token(&Some("ETH".to_string())));
        assert!(!tool.is_quote_token(&None));
    }

    #[test]
    fn test_quote_priority() {
        let tool = TokenReorderingTool::with_default_config();
        
        assert_eq!(tool.get_quote_priority(&Some("KTO".to_string())), 0);
        assert_eq!(tool.get_quote_priority(&Some("USDT".to_string())), 1);
        assert_eq!(tool.get_quote_priority(&Some("NOS".to_string())), 2);
        assert_eq!(tool.get_quote_priority(&Some("ETH".to_string())), usize::MAX);
    }

    #[test]
    fn test_should_swap_tokens() {
        let tool = TokenReorderingTool::with_default_config();
        
        // KTO作为token0，ETH作为token1，应该交换
        assert!(tool.should_swap_tokens(&Some("KTO".to_string()), &Some("ETH".to_string())));
        
        // ETH作为token0，KTO作为token1，不应该交换
        assert!(!tool.should_swap_tokens(&Some("ETH".to_string()), &Some("KTO".to_string())));
        
        // KTO优先级高于USDT，应该交换
        assert!(tool.should_swap_tokens(&Some("KTO".to_string()), &Some("USDT".to_string())));
        
        // USDT优先级低于KTO，不应该交换
        assert!(!tool.should_swap_tokens(&Some("USDT".to_string()), &Some("KTO".to_string())));
    }
}
