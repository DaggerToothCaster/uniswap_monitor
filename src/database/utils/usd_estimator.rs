...
mod tests {
    ...
    #[test]
    fn test_estimate_trade_volume_usd_buy() {
        let estimator = UsdEstimator::with_default_config();
        
        let result = estimator.estimate_trade_volume_usd(
            Decimal::from(100), // amount0_in
            Decimal::ZERO,      // amount1_in
            Decimal::ZERO,      // amount0_out
            Decimal::from(50),  // amount1_out
            "buy",
            Some(Decimal::from(2000)), // token0_price (ETH = $2000)
            None,                      // token1_price
            true,                      // token0_has_price
            false,                     // token1_has_price
            &Some("ETH".to_string()),
            &Some("USDT".to_string()),
        );
        
        assert!(result.is_valid);
        assert_eq!(result.usd_value, Decimal::from(200000)); // 100 * 2000
        assert_eq!(result.estimation_method, "buy_with_token0_price");
    }

    #[test]
    fn test_estimate_liquidity_event_usd_with_quote_token() {
        let estimator = UsdEstimator::with_default_config();
        let mut price_map = HashMap::new();
        price_map.insert("USDT".to_string(), Decimal::from(1));
        
        let result = estimator.estimate_liquidity_event_usd(
            Decimal::from(100), // amount0 (ETH)
            Decimal::from(200000), // amount1 (USDT)
            &Some("ETH".to_string()),
            &Some("USDT".to_string()),
            &price_map,
        );
        ...
        assert!(result.is_valid);
        assert_eq!(result.usd_value, Decimal::from(400000)); // 200000 * 1 * 2
        assert_eq!(result.estimation_method, "quote_token1_liquidity_doubled");
    }

    #[test]
    fn test_estimation_statistics() {
        let results = vec![
            UsdEstimationResult::new(
                Decimal::from(1000),
                "method1".to_string(),
        ...
        let estimator = UsdEstimator::with_default_config();
        let stats = estimator.calculate_estimation_statistics(&results);
        
        assert_eq!(stats.total_items, 3);
        assert_eq!(stats.valid_items, 2);
        assert_eq!(stats.total_usd_value, Decimal::from(3000));
        assert_eq!(stats.avg_usd_value, Decimal::from(1500));
        assert_eq!(stats.validity_rate(), 2.0 / 3.0);
        assert_eq!(stats.most_common_method(), Some("method1".to_string()));
    }
}
