...
impl TokenAmount {
    ...
    pub fn format(&self, precision: Option<usize>) -> String {
        ...
        if let Some(ref symbol) = self.symbol {
            format!("{:.precision$} {}", self.actual_amount, symbol, precision = precision)
        } else {
...
impl AmountConverter {
    ...
    /// 批量转换原始金额
    pub fn batch_convert_raw_to_actual(
        raw_amounts: &[i64],
        decimals: i32,
    ) -> Vec<Decimal> {
        raw_amounts.iter()
    ...
    /// 批量创建代币金额对象
    pub fn batch_create_token_amounts(
        data: &[(i64, i32, Option<String>)],
    ) -> Vec<TokenAmount> {
        data.iter()
        ...
        amounts.iter()
            .map(|amount| {
                if amount.decimals == target_decimals {
                    ...
                    // 需要增加精度
                    let multiplier = Decimal::from(10_i64.pow((target_decimals - amount.decimals) as u32));
                    amount.actual_amount * multiplier
                } else {
                    // 需要减少精度
                    let divisor = Decimal::from(10_i64.pow((amount.decimals - target_decimals) as u32));
    ...
    pub fn calculate_total_amount(amounts: &[TokenAmount]) -> Decimal {
        amounts.iter()
            .map(|amount| amount.actual_amount)
            .sum()
    }
    ...
    pub fn group_amounts_by_symbol(amounts: &[TokenAmount]) -> HashMap<String, Vec<TokenAmount>> {
        ...
        for amount in amounts {
            let symbol = amount.symbol.clone().unwrap_or_else(|| "UNKNOWN".to_string());
            grouped.entry(symbol).or_insert_with(Vec::new).push(amount.clone());
        }
...
impl TradeAmounts {
    ...
    pub fn total_output(&self) -> (Decimal, Decimal) {
        (self.amount0_out.actual_amount, self.amount1_out.actual_amount)
    }
