...
impl DataProcessor {
    ...
    /// 分页数据
    pub fn paginate<T: Clone>(data: Vec<T>, limit: Option<usize>, offset: Option<usize>) -> (Vec<T>, usize) {
        let total = data.len();
    ...
    pub fn detect_outliers(values: &[Decimal], threshold: f64) -> Vec<usize> {
        ...
        let threshold_decimal = Decimal::try_from(threshold)
            .unwrap_or(Decimal::from_f64(1.5).unwrap_or(Decimal::ZERO));
